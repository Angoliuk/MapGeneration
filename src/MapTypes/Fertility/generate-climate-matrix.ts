...
export const generateClimateMatrix = (): Array<Array<string>> => {
  ...
  for (let y = 0; y < mapSize.height; y++) {
    ...
    for (let x = 0; x < mapSize.width; x++) {
      ...
      const u = 1 - n;
      pixels[y][x] = `rgb(${ climateColors[reliefType].from.R * u + climateColors[reliefType].to.R * n }, ${ climateColors[reliefType].from.G * u + climateColors[reliefType].to.G * n }, ${ climateColors[reliefType].from.B * u + climateColors[reliefType].to.B * n })`;
    }
  }
  return pixels;
};
